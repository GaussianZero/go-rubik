...
func OLLCase(n int) rubik.Cube {
	return ollCases[n].Copy()
}
...
func OLLDeepMatch(c, d rubik.Cube) (bool, bool) {
	return OLLDebugDeepMatch(c, d, false)
}

func OLLMatch(c, d rubik.Cube) bool {
	return OLLDebugMatch(c, d, false)
}

func OLLDebugDeepMatch(c, d rubik.Cube, debug bool) (bool, bool) {
	s1, s2 := c.String(), d.String()
	if len(s1) == 0 || len(s2) == 0 {
		return false, false
	}

	// Check that all W pieces match
	for i, c := range s1 {
		// 119 == w, w is on top
		if c == 119 {
			if s2[i] != 119 {
	...
	fmt.Printf("deep match!\n\t%s\n\t%s\n", s1, s2)
	
	// Check that specific pieces match.
	if !rubik.F2LMatches(c, d) {
		fmt.Println("f2l didn't match")
		return true, false
	}
	// Bingo.
	return true, true
...
func OLLDebugMatch(c, d rubik.Cube, debug bool) bool {
	s1, s2 := c.String(), d.String()
